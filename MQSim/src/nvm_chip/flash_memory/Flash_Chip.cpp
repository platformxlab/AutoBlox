...
namespace NVM
{
	namespace FlashMemory
	{
		...
		void Flash_Chip::Report_results_in_XML(std::string name_prefix, Utils::XmlWriter& xmlwriter)
		{
			...
			attr = "Time_in_Idle";
			val = std::to_string(float(Simulator->Time()) - float(STAT_totalExecTime) - float(STAT_totalXferTime));
			xmlwriter.Write_attribute_string_inline(attr, val);

			attr = "Read_count";
			val = std::to_string(STAT_readCount);
			xmlwriter.Write_attribute_string_inline(attr, val);

			attr = "Program_count";
			val = std::to_string(STAT_progamCount);
			xmlwriter.Write_attribute_string_inline(attr, val);

			attr = "Erase_count";
			val = std::to_string(STAT_eraseCount);
			xmlwriter.Write_attribute_string_inline(attr, val);
