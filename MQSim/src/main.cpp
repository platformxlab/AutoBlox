...
int main(int argc, char* argv[])
{
	...
	for (auto io_scen = io_scenarios->begin(); io_scen != io_scenarios->end(); io_scen++, cntr++) {
		...
		std::string cache_output_filename = (*((*io_scen)->begin()))->Cache_Output_Filename;
		std::string best_queue_size = (*((*io_scen)->begin()))->Best_Queue_Size;
		std::string dram_output_filename = (*((*io_scen)->begin()))->Dram_Output_Filename;
		bool is_number = true;
		for (char &a : best_queue_size) {
			if (!(std::isdigit(a))) {
				PRINT_MESSAGE("WARNING: NON DIGIT " << a)
				is_number = false;
				break;
			}
		}
		exec_params->SSD_Device_Configuration.Queue_Fetch_Size = (uint16_t) 4;
		exec_params->SSD_Device_Configuration.IO_Queue_Depth = (uint16_t) 4;
		if (!is_number) {
			PRINT_MESSAGE("WARNING: Not valid IO Queue Depth! " << best_queue_size)
			PRINT_MESSAGE("Using Default Value!!!")
		} else{
			PRINT_MESSAGE("IO Queue Depth Set for Given Workload! " << best_queue_size)
			exec_params->SSD_Device_Configuration.Queue_Fetch_Size = (uint16_t) std::stoi(best_queue_size);
			// exec_params->SSD_Device_Configuration.IO_Queue_Depth = (uint16_t) std::stoi(best_queue_size);
		}
		SSD_Device ssd(&exec_params->SSD_Device_Configuration, &exec_params->Host_Configuration.IO_Flow_Definitions, cache_output_filename, dram_output_filename);//Create SSD_Device based on the specified parameters
		...
		// here we extract the cache file path of the first I/O flow as file path. This format should be consistent with the def file.
		std::string cache_input_filename = (*((*io_scen)->begin()))->Cache_Input_Filename;
		std::string device_throughput = (*((*io_scen)->begin()))->Device_Throughput;
		// check whether device throughput is a valid number
		is_number = true;
		for (char &a : device_throughput) {
		...
		double d_bandwidth = DEVICE_BANDWIDTH;
		if (!is_number) {
			PRINT_MESSAGE("WARNING: Not valid device throughput! " << device_throughput)
			PRINT_MESSAGE("Using Default Value.")
		} else{
			d_bandwidth = std::stod(device_throughput);
		}
		Host_System host(&exec_params->Host_Configuration, exec_params->SSD_Device_Configuration.Enabled_Preconditioning, ssd.Host_interface, cache_input_filename, d_bandwidth);
