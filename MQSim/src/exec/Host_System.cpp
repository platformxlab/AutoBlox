...
Host_System::Host_System(Host_Parameter_Set* parameters, bool preconditioning_required, SSD_Components::Host_Interface_Base* ssd_host_interface, std::string cache_input_filename, double device_throughput):
	MQSimEngine::Sim_Object("Host"), preconditioning_required(preconditioning_required), cache_input_filename(cache_input_filename)
{
		...
		switch (parameters->IO_Flow_Definitions[flow_id]->Type) {
			...
			case Flow_Type::TRACE: {
				...
				io_flow = new Host_Components::IO_Flow_Trace_Based(this->ID() + ".IO_Flow.Trace." + flow_param->File_Path, flow_id,
					...
					parameters->Enable_ResponseTime_Logging, parameters->ResponseTime_Logging_Period_Length, parameters->Input_file_path + ".IO_Flow.No_" + std::to_string(flow_id) + ".log", device_throughput);
...
void Host_System::Start_simulation()
{
	...
	if (preconditioning_required) {
		...
		ssd_device->Perform_preconditioning(workload_stats, cache_input_filename);
