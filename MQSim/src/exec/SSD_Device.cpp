...
SSD_Device::SSD_Device(Device_Parameter_Set *parameters, std::vector<IO_Flow_Parameter_Set *> *io_flows, std::string output_filename, std::string dram_filename) : MQSimEngine::Sim_Object("SSDDevice")
{
	...
	case NVM::NVM_Type::FLASH:
	{
		...
		std::string cmt_file_suffix = ".cmt";
		if (dram_filename == "") {
			cmt_file_suffix = "";
		}
		...
		case SSD_Components::Flash_Address_Mapping_Type::PAGE_LEVEL:
			amu = new SSD_Components::Address_Mapping_Unit_Page_Level(ftl->ID() + ".AddressMappingUnit", ftl, (SSD_Components::NVM_PHY_ONFI *)device->PHY,
																	  ...
																	  dram_filename + cmt_file_suffix, parameters->CMT_Sharing_Mode);
			break;
		...
		case SSD_Components::Caching_Mechanism::ADVANCED:
			dcm = new SSD_Components::Data_Cache_Manager_Flash_Advanced(device->ID() + ".DataCache", NULL, ftl, (SSD_Components::NVM_PHY_ONFI *)device->PHY,
																		...
																		parameters->Flash_Parameters.Page_Capacity / SECTOR_SIZE_IN_BYTE, parameters->Flash_Channel_Count * parameters->Chip_No_Per_Channel * parameters->Flash_Parameters.Die_No_Per_Chip * parameters->Flash_Parameters.Plane_No_Per_Die * parameters->Flash_Parameters.Page_Capacity / SECTOR_SIZE_IN_BYTE, output_filename, dram_filename);

			// DEBUG7("identifier calculate: " << parameters->Flash_Channel_Count << "  "<<  parameters->Chip_No_Per_Channel << "  "<<  parameters->Flash_Parameters.Die_No_Per_Chip << "  "<<  parameters->Flash_Parameters.Plane_No_Per_Die << "  "<<  parameters->Flash_Parameters.Page_Capacity << "/"<<  SECTOR_SIZE_IN_BYTE)
			break;
		default:
...
void SSD_Device::Perform_preconditioning(std::vector<Utils::Workload_Statistics *> workload_stats, std::string input_filename)
{
	if (Preconditioning_required)
	{
		...
		this->Cache_manager->Do_warmup(workload_stats, input_filename);
