...
void IO_Flow_Parameter_Set::XML_deserialize(rapidxml::xml_node<> *node)
{
	try {
		for (auto param = node->first_node(); param; param = param->next_sibling()) {
			...
			} else if (strcmp(param->name(), "Cache_Input_Filename") == 0) {
				Cache_Input_Filename = param->value();
			} else if (strcmp(param->name(), "Cache_Output_Filename") == 0) {
				Cache_Output_Filename = param->value();
			} else if (strcmp(param->name(), "Device_Throughput") == 0) {
				Device_Throughput = param->value();
			} else if (strcmp(param->name(), "Best_Queue_Size") == 0) {
				Best_Queue_Size = param->value();
			} else if (strcmp(param->name(), "Dram_Output_Filename") == 0) {
				Dram_Output_Filename = param->value();
			}
		}
	} catch (...) {
