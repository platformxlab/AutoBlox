         ...
         it++)
    {
        ...
        case Transaction_Type::READ:
            ...
            case Transaction_Source_Type::GC_WL:
                DEBUG6("GC WL transaction pushed. ")
        ...
        case Transaction_Type::WRITE:
            ...
            case Transaction_Source_Type::USERIO:
                if ((*it)->Priority_class != IO_Flow_Priority_Class::UNDEFINED)
                {
                    DEBUG7("DEBUG:: Write Transaction Queue: "<< (*it)->Address.ChannelID <<"  "<< (*it)->Address.ChipID <<" " << UserWriteTRQueue[(*it)->Address.ChannelID][(*it)->Address.ChipID][static_cast<int>((*it)->Priority_class)].size())
                    // write_request_enqueued += 1;
                    // DEBUG8("DEBUG:: Write Transaction Enqueued :" << write_request_enqueued)
                ...
                else
                {
                    // write_request_enqueued += 1;
                    // DEBUG8("DEBUG:: Write Transaction Enqueued :" << write_request_enqueued)
                    DEBUG7("DEBUG:: Write Transaction Queue: "<< (*it)->Address.ChannelID <<"  "<< (*it)->Address.ChipID <<" " << UserWriteTRQueue[(*it)->Address.ChannelID][(*it)->Address.ChipID][IO_Flow_Priority_Class::HIGH].size())
