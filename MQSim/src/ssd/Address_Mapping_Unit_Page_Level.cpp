...
namespace SSD_Components
{
	...
	Address_Mapping_Unit_Page_Level::Address_Mapping_Unit_Page_Level(const sim_object_id_type& id, FTL* ftl, NVM_PHY_ONFI* flash_controller, Flash_Block_Manager_Base* block_manager,
		...
		double Overprovisioning_ratio, std::string cmt_trace_filename, CMT_Sharing_Mode sharing_mode, bool fold_large_addresses)
		: Address_Mapping_Unit_Base(id, ftl, flash_controller, block_manager, ideal_mapping_table,
			...
			Block_no_per_plane, Page_no_per_block, SectorsPerPage, PageSizeInByte, Overprovisioning_ratio, sharing_mode, fold_large_addresses), cmt_trace_filename(cmt_trace_filename),
			cmt_tracing(false)
	{
		if (cmt_trace_filename != "") {
			cmt_tracing = true;
			std::cout << "CMT Tracing is ON! Saving to File: " << cmt_trace_filename << std::endl;
		} else {
			std::cout << "CMT Tracing is OFF! No FileName Provided: " << cmt_trace_filename << std::endl;
		}
		_my_instance = this;
	...
	Address_Mapping_Unit_Page_Level::~Address_Mapping_Unit_Page_Level()
	{
		if (cmt_tracing) {
			...
			cmt_trace_writer.open(cmt_trace_filename, std::ofstream::out);
			cmt_trace_writer << cmt_trace_data.str();
			cmt_trace_writer.close();
		}
	...
	// int submitted_to_mapping_table;

	// int submitted_to_CMT = 0;

	void Address_Mapping_Unit_Page_Level::Translate_lpa_to_ppa_and_dispatch(const std::list<NVM_Transaction*>& transactionList)
	{
		// submitted_to_mapping_table += transactionList.size();
		// DEBUG8("Submitted to mapping table:" << submitted_to_mapping_table);
		DEBUG7("In transaction submit. Size=" <<  transactionList.size())	
		...
		if (transactionList.size() > 0) {
			...
			for (std::list<NVM_Transaction*>::const_iterator it = transactionList.begin();
				...
				if (((NVM_Transaction_Flash*)(*it))->Physical_address_determined) {
					DEBUG7("Write transaction submited!!")
	...
	bool Address_Mapping_Unit_Page_Level::query_cmt(NVM_Transaction_Flash* transaction)
	{
		...
		{
			if (cmt_tracing) {
				cmt_trace_data << Simulator->Time() << " READ ";
				cmt_trace_data << transaction->LPA << " " << transaction->Data_and_metadata_size_in_byte << std::endl; 
			}
			...
			if (cmt_tracing) {
				cmt_trace_data << Simulator->Time() << " WRITE ";
				cmt_trace_data << transaction->LPA << " " << transaction->Data_and_metadata_size_in_byte << std::endl; 
			}
			//Maybe we can catch mapping data from an on-the-fly write back request
			// DEBUG4("querying reansaction PPA = " << transaction->PPA)
			if (request_mapping_entry(stream_id, transaction->LPA)) {
				...
				if (transaction->Type == Transaction_Type::READ) {\
					...
					DEBUG7("DEBUG:: CMT 6 Waiting SIze=" << domains[stream_id]->Waiting_unmapped_program_transactions.size())
					// submitted_to_CMT += 1;
					// DEBUG8("DEBUG:: CMT 6, submitted=" << submitted_to_CMT)
					Stats::total_writeTR_CMT_queries++;
					Stats::total_writeTR_CMT_queries_per_stream[stream_id]++;
					Stats::writeTR_CMT_miss++;
					Stats::writeTR_CMT_miss_per_stream[stream_id]++;
	...
	*/
	bool Address_Mapping_Unit_Page_Level::translate_lpa_to_ppa(stream_id_type streamID, NVM_Transaction_Flash* transaction)
	{
		...
		if (transaction->Type == Transaction_Type::READ) {
			...
			// DEBUG8("In write allocate plane! ")
			...
			for (unsigned int chip_cntr = 0; chip_cntr < domains[stream_id]->Chip_no; chip_cntr++) {
				for (unsigned int die_cntr = 0; die_cntr < domains[stream_id]->Die_no; die_cntr++) {
					for (unsigned int plane_cntr = 0; plane_cntr < domains[stream_id]->Plane_no; plane_cntr++) {
						...
						// get total available block in the plane
						...
						//Adjust the average
						// average means average blocks in use.
						double model_average = 0;
						std::vector<double> adjusted_steady_state_distribution;
						//Check if probability distribution is correct 
						ss << "Input distribution:" << std::endl;
						DEBUG11("Address Distribution for Channel:" << plane_address.ChannelID << " Chip:" << plane_address.ChipID << " Die:" << plane_address.DieID << " Plane:" << plane_address.PlaneID)
						for (unsigned int i = 0; i <= pages_no_per_block; i++) {
							ss << i << ":" <<steady_state_distribution[i] << "; ";
						...
						ss << std::endl;
						double real_average = double(assigned_lpas[plane_address.ChannelID][plane_address.ChipID][plane_address.DieID][plane_address.PlaneID].size()) / (physical_block_consumption_goal);
						if (std::abs(model_average - real_average)  > 0.9999) {
							int displacement_index = int((real_average - model_average));
							if (displacement_index > 0) {
								int page_difference = 0;
								page_difference += assigned_lpas[plane_address.ChannelID][plane_address.ChipID][plane_address.DieID][plane_address.PlaneID].size();
								for(int i = 1; i <= int(pages_no_per_block); i++) {
									for (int j = 0; j <= int(pages_no_per_block); j++) {
										adjusted_steady_state_distribution[j] = 0;
									}
									for (int j = 0; j <= int(pages_no_per_block) - i; j++) {
										adjusted_steady_state_distribution[j + i] += steady_state_distribution[j];
									}
									for (int j =  int(pages_no_per_block) - i; j <= int(pages_no_per_block); j++) {
										adjusted_steady_state_distribution[int(pages_no_per_block)] += steady_state_distribution[j];
									}
									int tpage = 0;
									for (unsigned int i = 0; i <= pages_no_per_block; i++) {
										tpage += i * (std::round(adjusted_steady_state_distribution[i] * physical_block_consumption_goal));
									}
									if (tpage >= int(pages_no_per_block) + page_difference) {
										break;
									}
								}
							} else {
								displacement_index *= -1;
								DEBUG9("Previous index "<< displacement_index)
								int page_difference = 0;
								page_difference += assigned_lpas[plane_address.ChannelID][plane_address.ChipID][plane_address.DieID][plane_address.PlaneID].size();
								for(int i = 1; i <= int(pages_no_per_block); i++) {
									for (int j = 0; j <= int(pages_no_per_block); j++) {
									...
									for (int j = i; j <= int(pages_no_per_block); j++) {
										adjusted_steady_state_distribution[j - i] += steady_state_distribution[j];
									}
									for (int j =  0; j < i; j++) {
										adjusted_steady_state_distribution[0] += steady_state_distribution[j];
									}
									unsigned int tpage = 0;
									for (unsigned int i = 0; i <= pages_no_per_block; i++) {
										tpage += i * ((unsigned int)std::round(adjusted_steady_state_distribution[i] * physical_block_consumption_goal));
									}
									if (int(tpage)  < 2 * int(pages_no_per_block) + page_difference) {
										break;
									}
								}
							}
						...
						for (int valid_pages_in_block = pages_no_per_block; valid_pages_in_block >= 0; valid_pages_in_block--) {
							total_valid_pages += valid_pages_in_block * (unsigned int)std::round(adjusted_steady_state_distribution[valid_pages_in_block] * physical_block_consumption_goal);
						}
						...
						unsigned int blocks_abandoned = 0;
						unsigned int pages_abandoned = 0;
						unsigned int blocks_added_by_free_PPA = 0;
						unsigned int add_ppa_blocksize = 0;
						unsigned int prev_pages_need_ppa = pages_need_PPA;
						unsigned int calculate_maxblocknum = 0;
						unsigned int calculate_remainblock = 0;
						// first check whether the block allocation is safe.
						DEBUG11("Page Needed:" << pages_need_PPA << " , Page Have:" << total_valid_pages)
						for (int valid_pages_in_block = pages_no_per_block; valid_pages_in_block >= 0; valid_pages_in_block--) {
							unsigned int block_no_with_x_valid_page = (unsigned int)std::round(adjusted_steady_state_distribution[valid_pages_in_block] * physical_block_consumption_goal);
							if (block_no_with_x_valid_page > 0 && pages_need_PPA > 0) {
								add_ppa_blocksize = valid_pages_in_block;
								blocks_added_by_free_PPA = (pages_need_PPA / valid_pages_in_block) + (pages_need_PPA % valid_pages_in_block == 0 ? 0 : 1);
								block_no_with_x_valid_page += (pages_need_PPA / valid_pages_in_block) + (pages_need_PPA % valid_pages_in_block == 0 ? 0 : 1);
								pages_need_PPA = 0;
								// check, if there is still enough blocks:
								unsigned int blocks_needed = 0;
								for (int check = valid_pages_in_block - 1; check >= 0; check--) {
									unsigned int blocknum_check = (unsigned int)std::round(adjusted_steady_state_distribution[check] * physical_block_consumption_goal);
									blocks_needed += blocknum_check;
								}
								calculate_maxblocknum = blocks_needed + block_no_with_x_valid_page;
								calculate_remainblock = remaining_blocks_to_consume;
								if (blocks_needed + block_no_with_x_valid_page > remaining_blocks_to_consume) {
									bool can_continue = false;
									while(!can_continue) {
										block_no_with_x_valid_page += 1;
										unsigned int total_pages = block_no_with_x_valid_page * valid_pages_in_block;
										unsigned int total_blocks = block_no_with_x_valid_page;
										// assigned_lpas[plane_address.ChannelID][plane_address.ChipID][plane_address.DieID][plane_address.PlaneID].size()
										for (int check = valid_pages_in_block; check >= 0; check--) {
											unsigned int blocknum_check = (unsigned int)std::round(adjusted_steady_state_distribution[valid_pages_in_block] * physical_block_consumption_goal) ;
											if (total_blocks + blocknum_check > remaining_blocks_to_consume) {
												blocknum_check = remaining_blocks_to_consume - total_blocks;
												total_blocks = remaining_blocks_to_consume;
											}
											total_pages += blocknum_check * check;
										}
										if (total_pages >= assigned_lpas[plane_address.ChannelID][plane_address.ChipID][plane_address.DieID][plane_address.PlaneID].size()) {
											can_continue = true;
										}
									}
								}
							...
							if (block_no_with_x_valid_page <= remaining_blocks_to_consume) {
								...
								blocks_abandoned += block_no_with_x_valid_page - remaining_blocks_to_consume;
								pages_abandoned += blocks_abandoned * (unsigned int)valid_pages_in_block;
								block_no_with_x_valid_page = remaining_blocks_to_consume;
								remaining_blocks_to_consume = 0;
							}
							// DEBUG9("Block No with x valid page"<<block_no_with_x_valid_page)
							for (int block_cntr = 0; block_cntr < block_no_with_x_valid_page; block_cntr++) {
						...
						ss << "Address Distribution for Channel:" << plane_address.ChannelID << " Chip:" << plane_address.ChipID << " Die:" << plane_address.DieID << " Plane:" << plane_address.PlaneID << std::endl;
						ss << "**************distribution display:\n";
						for (int valid_pages_in_block = pages_no_per_block; valid_pages_in_block >= 0; valid_pages_in_block--) {
							unsigned int block_no_with_x_valid_page = (unsigned int)std::round(adjusted_steady_state_distribution[valid_pages_in_block] * physical_block_consumption_goal);
							ss << valid_pages_in_block << ":" << block_no_with_x_valid_page << " ; ";
						}
						DEBUG11("Address Distributed for Channel:" << plane_address.ChannelID << " Chip:" << plane_address.ChipID << " Die:" << plane_address.DieID << " Plane:" << plane_address.PlaneID)
						if (assigned_lpas[plane_address.ChannelID][plane_address.ChipID][plane_address.DieID][plane_address.PlaneID].size() > 0) {
							PRINT_ERROR("It is not possible to assign PPA to all LPAs in Allocate_address_for_preconditioning! block abandoned:"
							<< blocks_abandoned << " pages abandoned:" << pages_abandoned << " blocks_added_by_free_PPA:" 
							<< blocks_added_by_free_PPA << " add_ppa_blocksize:" << add_ppa_blocksize 
							<< " pages_need_PPA:" << prev_pages_need_ppa<<" It is not safe to continue preconditioning." 
							<< " calculate_maxblocknum:" << calculate_maxblocknum
							<< " calculate_remainblock" <<  remaining_blocks_to_consume << " "
							<< assigned_lpas[plane_address.ChannelID][plane_address.ChipID][plane_address.DieID][plane_address.PlaneID].size() << ss.str());
						}
					}
				}
			}
		}
		DEBUG11("Allocation is finished.")
		for (unsigned int channel_cntr = 0; channel_cntr < channel_count; channel_cntr++) {
			for (unsigned int chip_cntr = 0; chip_cntr < chip_no_per_channel; chip_cntr++) {
				for (unsigned int die_cntr = 0; die_cntr < die_no_per_chip; die_cntr++) {
	...
	inline MVPN_type Address_Mapping_Unit_Page_Level::get_MVPN(const LPA_type lpn, stream_id_type stream_id)
	{
		...
		return (MVPN_type)(lpn / (uint64_t)no_of_translation_entries_per_page);
	}
	...
	bool Address_Mapping_Unit_Page_Level::request_mapping_entry(const stream_id_type stream_id, const LPA_type lpa)
	{
		...
		if (mvpn > domain->Total_translation_pages_no) {
			DEBUG4("MVPN = " << mvpn << "; LPA = " << lpa <<"; no_of_translation_entries_per_page = " << no_of_translation_entries_per_page << "; Total_translation_pages_no = " << domain->Total_translation_pages_no << "; Translation_entries_per_page = " << domain->Translation_entries_per_page)
		}
	...
	void Address_Mapping_Unit_Page_Level::generate_flash_writeback_request_for_mapping_data(const stream_id_type stream_id, const LPA_type lpn)
	{
		...
		if (is_mvpn_locked_for_gc(stream_id, mvpn)) {
			...
			if (mppn != NO_MPPN) {
				// DEBUG4("read request 1 with LPA = " << mvpn)
	...
	void Address_Mapping_Unit_Page_Level::generate_flash_read_request_for_mapping_data(const stream_id_type stream_id, const LPA_type lpn)
	{
		...
		if (is_mvpn_locked_for_gc(stream_id, mvpn)) {
			...
			NVM_Transaction_Flash_RD* readTR = new NVM_Transaction_Flash_RD(Transaction_Source_Type::MAPPING, stream_id,
					SECTOR_SIZE_IN_BYTE, mvpn, NO_PPA, NULL, mvpn, ((page_status_type)0x1) << sector_no_per_page, CurrentTimeStamp);
			Convert_ppa_to_address(ppn, readTR->Address);
			block_manager->Read_transaction_issued(readTR->Address);//Inform block_manager as soon as the transaction's target address is determined
			readTR->PPA = ppn;
			// DEBUG4("read request 2 with PPN = " << ppn << "LPN = " << lpn)
			ftl->TSU->Submit_transaction(readTR);
	...
	// int request_erased = 0;
	// int unsuccessful_erased = 0;
						...
						while (it2 != _my_instance->domains[transaction->Stream_id]->Waiting_unmapped_program_transactions.end() &&
							...
							if (_my_instance->is_lpa_locked_for_gc(transaction->Stream_id, lpa)) {
								...
								if (_my_instance->translate_lpa_to_ppa(transaction->Stream_id, it2->second)) {
									...
									// unsuccessful_erased += 1;
									// DEBUG8("Uncscuucessful Erased from CMT 6 " << unsuccessful_erased)
									_my_instance->mange_unsuccessful_translation(it2->second);
								}
							}
							// request_erased += 1;
							// DEBUG8("Request Erased from CMT 6 " << request_erased)
	...
	inline void Address_Mapping_Unit_Page_Level::Set_barrier_for_accessing_lpa(stream_id_type stream_id, LPA_type lpa)
	{
		...
		if (itr != domains[stream_id]->Locked_LPAs.end()) {
			PRINT_ERROR("Illegal operation: Locking an LPA that has already been locked!" << lpa);
		}
	...
	inline void Address_Mapping_Unit_Page_Level::Set_barrier_for_accessing_physical_block(const NVM::FlashMemory::Physical_Page_Address& block_address)
	{
		...
		DEBUG6("Locking: ChannelChipDiePlaneBlock: " << block_address.ChannelID << " " << block_address.ChipID << " " << block_address.DieID << " " << block_address.PlaneID << " " << block_address.BlockID)
		NVM::FlashMemory::Physical_Page_Address addr(block_address);
		for (flash_page_ID_type pageID = 0; pageID < block->Current_page_write_index; pageID++) {
			if (block_manager->Is_page_valid(block, pageID)  && (MVPN_type)flash_controller->Get_metadata(addr.ChannelID, addr.ChipID, addr.DieID, addr.PlaneID, addr.BlockID, addr.PageID) != NO_PPA) {
				addr.PageID = pageID;
				if (block->Holds_mapping_data) {
					...
					DEBUG6("mvpn for locking block:" << mpvn)
					...
					if (ppa != Convert_address_to_ppa(addr)) {
						...
						domains[block->Stream_id]->GlobalMappingTable[lpa].PPA = ppa;
						if (domains[block->Stream_id]->Mapping_entry_accessible(ideal_mapping_table, block->Stream_id, lpa)) {
							// Stats::CMT_hits++;
							// Stats::CMT_hits_per_stream[block->Stream_id]++;
							// Stats::total_writeTR_CMT_queries++;
							// Stats::total_writeTR_CMT_queries_per_stream[block->Stream_id]++;
							// Stats::writeTR_CMT_hits++;
							// Stats::writeTR_CMT_hits_per_stream[block->Stream_id]++;
							domains[block->Stream_id]->Update_mapping_info(ideal_mapping_table, block->Stream_id, lpa, ppa, 0);
						} else { //the else block only executed for non-ideal mapping table in which CMT has a limited capacity and mapping data is read/written from/to the flash storage
							if (!domains[block->Stream_id]->CMT->Check_free_slot_availability()) {
								LPA_type evicted_lpa;
								CMTSlotType evictedItem = domains[block->Stream_id]->CMT->Evict_one_slot(evicted_lpa);
								if (evictedItem.Dirty) {
									/* In order to eliminate possible race conditions for the requests that
									* will access the evicted lpa in the near future (before the translation
									* write finishes), MQSim updates GMT (the on flash mapping table) right
									* after eviction happens.*/
									domains[block->Stream_id]->GlobalMappingTable[evicted_lpa].PPA = evictedItem.PPA;
									domains[block->Stream_id]->GlobalMappingTable[evicted_lpa].WrittenStateBitmap = evictedItem.WrittenStateBitmap;
									if (domains[block->Stream_id]->GlobalMappingTable[evicted_lpa].TimeStamp > CurrentTimeStamp)
										throw std::logic_error("Unexpected situation occured in handling GMT!");
									domains[block->Stream_id]->GlobalMappingTable[evicted_lpa].TimeStamp = CurrentTimeStamp;
									generate_flash_writeback_request_for_mapping_data(block->Stream_id, evicted_lpa);
								}
							}
							domains[block->Stream_id]->CMT->Reserve_slot_for_lpn(block->Stream_id, lpa);
							domains[block->Stream_id]->CMT->Insert_new_mapping_info(block->Stream_id, lpa, ppa, 0);
						}
					}
					Set_barrier_for_accessing_lpa(block->Stream_id, lpa);
					DEBUG6("Locking: LPA = " << lpa)
				}
			}
		}
	}
	...
	{
		DEBUG6("Removing LPA Lock for LPA = " << lpa)
		auto itr = domains[stream_id]->Locked_LPAs.find(lpa);
		if (itr == domains[stream_id]->Locked_LPAs.end()) {
			DEBUG5("Illegal operation: Unlocking an LPA that has not been locked!");
			return;
		...
		while (write_tr != domains[stream_id]->Write_transactions_behind_LPA_barrier.end()) {
			DEBUG7("Remove Barrier for lpa=" << lpa)
	...
	inline void Address_Mapping_Unit_Page_Level::manage_user_transaction_facing_barrier(NVM_Transaction_Flash* transaction)
	{
		...
		if (transaction->Type == Transaction_Type::READ) {
			...
			DEBUG7("Facing Barrier! Insert to wait LPA=" << transaction->LPA <<" queue length=" << domains[transaction->Stream_id]->Write_transactions_behind_LPA_barrier.size())
	...
	void Address_Mapping_Unit_Page_Level::mange_unsuccessful_translation(NVM_Transaction_Flash* transaction)
	{
		...
		DEBUG7("Unsccessful Transaction! " << transaction->Address.ChannelID << " " << transaction->Address.ChipID << " " << transaction->Address.DieID << " " << transaction->Address.PlaneID << "Size=" << Write_transactions_for_overfull_planes[transaction->Address.ChannelID][transaction->Address.ChipID][transaction->Address.DieID][transaction->Address.PlaneID].size())
	...
	// int serviced_from_unsuccessful = 0;
	void Address_Mapping_Unit_Page_Level::Start_servicing_writes_for_overfull_plane(const NVM::FlashMemory::Physical_Page_Address plane_address)
	{
		...
		while (program != waiting_write_list.end()) {
			// DEBUG8("Servicing from unsuccessful list! ")
			if (translate_lpa_to_ppa((*program)->Stream_id, *program)) {
				// serviced_from_unsuccessful += 1;
				// DEBUG8("Serviced from unsuccessful list! num=" << serviced_from_unsuccessful)
