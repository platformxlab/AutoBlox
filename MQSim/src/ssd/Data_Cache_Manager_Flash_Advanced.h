	...
	*/
	...
	public:
		Data_Cache_Manager_Flash_Advanced(const sim_object_id_type& id, Host_Interface_Base* host_interface, NVM_Firmware* firmware, NVM_PHY_ONFI* flash_controller,
			...
			unsigned int stream_count, unsigned int sector_no_per_page, unsigned int back_pressure_buffer_max_depth,std::string Output_Filename, std::string DRAM_Filename);
		~Data_Cache_Manager_Flash_Advanced();
		void Execute_simulator_event(MQSimEngine::Sim_Event* ev);
		void Setup_triggers();
		void Do_warmup(std::vector<Utils::Workload_Statistics*> workload_stats, std::string input_filename);
	private:
		...
		std::string dramtrace_filename;
		std::ofstream dram_writer;
		std::stringstream dram_data;
		bool dram_tracing;
