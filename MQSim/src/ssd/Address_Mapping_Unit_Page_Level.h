	...
	* its own share of CMT (equal partitioning of CMT space among concurrent streams).*/
	...
	public:
		Address_Mapping_Unit_Page_Level(const sim_object_id_type& id, FTL* ftl, NVM_PHY_ONFI* flash_controller, Flash_Block_Manager_Base* block_manager,
			...
			double Overprovisioning_ratio, std::string cmt_trace_filename, CMT_Sharing_Mode sharing_mode = CMT_Sharing_Mode::SHARED, bool fold_large_addresses = true);
	...
	private:
		...
		std::string cmt_trace_filename;
		std::ofstream cmt_trace_writer;
		std::stringstream cmt_trace_data;
		bool cmt_tracing;
