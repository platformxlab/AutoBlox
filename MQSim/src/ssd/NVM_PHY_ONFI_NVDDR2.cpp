...
namespace SSD_Components {
	...
	void NVM_PHY_ONFI_NVDDR2::Send_command_to_chip(std::list<NVM_Transaction_Flash*>& transaction_list)
	{
		...
		* then something illegarl is happening*/
		...
		for (std::list<NVM_Transaction_Flash*>::iterator it = transaction_list.begin();
			...
			metadata.LPA = (*it)->LPA;
			DEBUG7("PushBackCommmands of Address:" << metadata.LPA)
		...
		switch (transaction_list.front()->Type) {
			case Transaction_Type::READ:
				if (transaction_list.size() == 1) {
					...
					DEBUG7("Chip " << targetChip->ChannelID << ", " << targetChip->ChipID << ", " << transaction_list.front()->Address.DieID << ": Sending read command to chip for LPA: " << transaction_list.front()->LPA)
				} else {
					Stats::IssuedMultiplaneReadCMD++;
					dieBKE->ActiveCommand->CommandCode = CMD_READ_PAGE_MULTIPLANE;
					DEBUG7("Chip " << targetChip->ChannelID << ", " << targetChip->ChipID << ", " << transaction_list.front()->Address.DieID << ": Sending multi-plane read command to chip for LPA: " << transaction_list.front()->LPA)
				}
				...
				break;
			case Transaction_Type::WRITE:
				// write_command_here += 1;
				// DEBUG8("Write Command Issued to q:" << write_command_here)
				if (((NVM_Transaction_Flash_WR*)transaction_list.front())->ExecutionMode == WriteExecutionModeType::SIMPLE) {
					if (transaction_list.size() == 1) {
						...
						DEBUG7("Chip " << targetChip->ChannelID << ", " << targetChip->ChipID << ", " << transaction_list.front()->Address.DieID << ": Sending program command to chip for LPA: " << transaction_list.front()->LPA)
					} else {
						Stats::IssuedMultiplaneProgramCMD++;
						dieBKE->ActiveCommand->CommandCode = CMD_PROGRAM_PAGE_MULTIPLANE;
						DEBUG7("Chip " << targetChip->ChannelID << ", " << targetChip->ChipID << ", " << transaction_list.front()->Address.DieID << ": Sending multi-plane program command to chip for LPA: " << transaction_list.front()->LPA)
					}
					...
					if (transaction_list.size() == 1) {
						...
						DEBUG7("Chip " << targetChip->ChannelID << ", " << targetChip->ChipID << ", " << transaction_list.front()->Address.DieID << ": Sending copy back program command to chip for LPA: " << transaction_list.front()->LPA)
					} else {
						Stats::IssuedMultiplaneCopybackProgramCMD++;
						dieBKE->ActiveCommand->CommandCode = CMD_READ_PAGE_COPYBACK_MULTIPLANE;
						DEBUG7("Chip " << targetChip->ChannelID << ", " << targetChip->ChipID << ", " << transaction_list.front()->Address.DieID << ": Sending multi-plane copy back program command to chip for LPA: " << transaction_list.front()->LPA)
					}
				...
				break;
			case Transaction_Type::ERASE:
				DEBUG7("Chip " << targetChip->ChannelID << ", " << targetChip->ChipID << ", " << transaction_list.front()->Address.DieID << ": Sending erase command to chip")
				if (transaction_list.size() == 1) {
	...
	void check_active_command(NVM::FlashMemory::Flash_Command* command) {
		int i = 0;
		for (auto &address : command->Address) {
			DEBUG4("Check Command, Medatata " << i << ":" << command->Meta_data[i].LPA)
			i++;
		}
	}
	...
	void copy_read_data_to_transaction(NVM_Transaction_Flash_RD* read_transaction, NVM::FlashMemory::Flash_Command* command)
	{
		...
		for (auto &address : command->Address) {
			if (address.PlaneID == read_transaction->Address.PlaneID) {
				DEBUG5("LPA2:" << read_transaction->LPA << " " << command->Meta_data[i].LPA)
				if (command->Meta_data[i].LPA == NO_LPA) {
					command->Meta_data[i].LPA = read_transaction->LPA;
				} else if (read_transaction->LPA == NO_LPA){
	...
	void NVM_PHY_ONFI_NVDDR2::Execute_simulator_event(MQSimEngine::Sim_Event* ev)
	{
		...
		DEBUG4("Channel:" << dieBKE->ActiveCommand->Address[0].ChannelID <<" Chip:" << dieBKE->ActiveCommand->Address[0].ChipID <<  " Die:" << dieBKE->ActiveCommand->Address[0].DieID)
		check_active_command(dieBKE->ActiveCommand);
		...
		switch ((NVDDR2_SimEventType)ev->Type) {
			case NVDDR2_SimEventType::READ_CMD_ADDR_TRANSFERRED:
				DEBUG7("Chip " << targetChip->ChannelID << ", " << targetChip->ChipID << ", " << dieBKE->ActiveTransactions.front()->Address.DieID << ": READ_CMD_ADDR_TRANSFERRED Simulated")
	...
	inline void NVM_PHY_ONFI_NVDDR2::handle_ready_signal_from_chip(NVM::FlashMemory::Flash_Chip* chip, NVM::FlashMemory::Flash_Command* command)
	{
		...
		case CMD_PROGRAM_PAGE_COPYBACK_MULTIPLANE:
		{
				...
				it != dieBKE->ActiveTransactions.end(); it++, i++)
			{
				...
				DEBUG4("LPA3:" << command->Meta_data[i].LPA)
	...
	{
		DEBUG5("Chip " << tr->Address.ChannelID << ", " << tr->Address.ChipID << ": transfer read data started for LPA: " << tr->LPA)
