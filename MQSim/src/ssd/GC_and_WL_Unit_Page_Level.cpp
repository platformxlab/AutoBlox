...
namespace SSD_Components
{
	...
	void GC_and_WL_Unit_Page_Level::Check_gc_required(const unsigned int free_block_pool_size, const NVM::FlashMemory::Physical_Page_Address& plane_address)
	{
		if (free_block_pool_size < block_pool_gc_threshold) {
			DEBUG6("GC is triggered in Channel:" << plane_address.ChannelID << " Chip:" << plane_address.ChipID << " Die:" << plane_address.DieID << " Plane:" << plane_address.PlaneID)
			DEBUG6("GC parameter: free_block_pool_size = " << free_block_pool_size << ", block_pool_gc_threshold = " << block_pool_gc_threshold)
