...
namespace SSD_Components
{
	...
	Data_Cache_Slot_Type Data_Cache_Flash::Get_next_slot()
	{
		assert(slots.size() > 0);
		Data_Cache_Slot_Type evicted_item = *lru_list.back().second;
		return evicted_item;
	...
	bool Data_Cache_Flash::Delete_front() {
		if (slots.size() == 0) {
			return false;
		}
		slots.erase(lru_list.back().first);
	...
	void Data_Cache_Flash::Insert_warmup_data(const stream_id_type stream_id, const LPA_type lpn, const data_cache_content_type content,
		const data_timestamp_type timestamp, const page_status_type state_bitmap_of_read_sectors, const Cache_Slot_Status status)
	{
		// DEBUG4("inserting read LPN = " << lpn)
		LPA_type key = LPN_TO_UNIQUE_KEY(stream_id, lpn);
		...
		if (slots.size() >= capacity_in_pages) {
			auto key1 = lru_list.back().first;
			auto it = slots.find(key1);
			slots.erase(key1);
			lru_list.pop_back();
		}
	...
	{
		// DEBUG4("inserting write LPN = " << lpn)
		LPA_type key = LPN_TO_UNIQUE_KEY(stream_id, lpn);
