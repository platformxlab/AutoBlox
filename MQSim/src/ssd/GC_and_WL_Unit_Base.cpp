...
namespace SSD_Components
{
		...
		dynamic_wearleveling_enabled(dynamic_wearleveling_enabled), static_wearleveling_enabled(static_wearleveling_enabled), static_wearleveling_threshold(static_wearleveling_threshold)
	{
		...
		DEBUG3("setting gc threshold: block_pool_gc_threshold = " << block_pool_gc_threshold << "; max_ongoing_gc_reqs_per_plane = " << max_ongoing_gc_reqs_per_plane
										<< "; gc_threshold = " << gc_threshold << "; block_no_per_plane = " << block_no_per_plane)
		...
		switch (transaction->Type) {
			case Transaction_Type::READ:
			{
				...
				if (pbke->Blocks[transaction->Address.BlockID].Holds_mapping_data) {
					...
					DEBUG5("lc 1 ppa = " << mppa << "transaction PPA = " << transaction->PPA)
					...
					//There has been no write on the page since GC start, and it is still valid
					DEBUG5("lc 2 ppa = " << ppa << "transaction PPA = " << transaction->PPA)
					transaction->PPA = ppa;
			...
			case Transaction_Type::WRITE:
				if (((NVM_Transaction_Flash_WR*)transaction)->RelatedErase != nullptr && pbke->Blocks[((NVM_Transaction_Flash_WR*)transaction)->RelatedErase->Address.BlockID].Holds_mapping_data) {
					_my_instance->address_mapping_unit->Remove_barrier_for_accessing_mvpn(transaction->Stream_id, (MVPN_type)transaction->LPA);
					DEBUG5(Simulator->Time() << ": MVPN=" << (MVPN_type)transaction->LPA << " unlocked!!");
				} else {
					DEBUG6(Simulator->Time() << ": LPA=" << (MVPN_type)transaction->LPA << " trying to unlock.");
					_my_instance->address_mapping_unit->Remove_barrier_for_accessing_lpa(transaction->Stream_id, transaction->LPA);
					DEBUG6(Simulator->Time() << ": LPA=" << (MVPN_type)transaction->LPA << " unlocked!!");
				}
				if (((NVM_Transaction_Flash_WR*)transaction)->RelatedErase != nullptr) 
	...
	{
		DEBUG6("In Base static wearleveling in Channel:" << plane_address.ChannelID << " Chip:" << plane_address.ChipID << " Die:" << plane_address.DieID << " Plane:" << plane_address.PlaneID)
		// DEBUG6("In Base GC parameter: free_block_pool_size = " << free_block_pool_size << ", block_pool_gc_threshold = " << block_pool_gc_threshold)	
		...
		if (block_manager->Can_execute_gc_wl(wl_candidate_address)) {//If there are ongoing requests targeting the candidate block, the gc execution should be postponed
			...
			DEBUG6("In Base static wearleveling, transaction issued.")
