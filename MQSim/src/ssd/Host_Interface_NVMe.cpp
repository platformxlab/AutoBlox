...
{
	// DEBUG7("DEBUG:: Submission queue update Stream ID:" << stream_id << " On the fly request: " << ((Input_Stream_NVMe *)input_streams[stream_id])->On_the_fly_requests
	// 													<< " Submission Tail : " << ((Input_Stream_NVMe *)input_streams[stream_id])->Submission_tail << " -> " << tail_pointer_value
	// 													<< " Submission Head : " << ((Input_Stream_NVMe *)input_streams[stream_id])->Submission_head
	// 													<< " On the fly Requests: " << ((Input_Stream_NVMe *)input_streams[stream_id])->On_the_fly_requests)
...
{
	// DEBUG7("Request Arrival at Handle new arrival requests.")
	...
	{ //This is a write request
		DEBUG7("Begin handle write requests.")
		((Input_Stream_NVMe *)input_streams[request->Stream_id])->Waiting_user_requests.push_back(request);
...
{
	// DEBUG7("Write data arrive at device.")
	segment_user_request(request);
	((Host_Interface_NVMe *)host_interface)->broadcast_user_request_arrival_signal(request);
}
...
inline void Input_Stream_Manager_NVMe::Handle_serviced_request(User_Request *request)
{
	...
	DEBUG7("** Host Interface: Request #" << request->ID << " from stream #" << request->Stream_id << " is finished")
...
void Request_Fetch_Unit_NVMe::Process_pcie_write_message(uint64_t address, void *payload, unsigned int payload_size)
{
	...
	// DEBUG7(" _____________ Delivered to submission queue..")
	switch (address)
	{
...
void Request_Fetch_Unit_NVMe::Process_pcie_read_message(uint64_t address, void *payload, unsigned int payload_size)
{
	...
	case DMA_Req_Type::REQUEST_INFO:
	{
		...
		// DEBUG7("DEBUG:: User request arrive at the device")
		switch (sqe->Opcode)
		{
		...
		case NVME_WRITE_OPCODE:
			...
			new_request->SizeInSectors = sqe->Command_specific[2] & (LHA_type)(0x0000ffff);
			new_request->Size_in_byte = new_request->SizeInSectors * SECTOR_SIZE_IN_BYTE;
			// DEBUG7("DEBUG:: User write request arrive at the device!!!")
			break;
		default:
