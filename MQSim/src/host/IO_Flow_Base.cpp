																												...
																												enabled_logging(enabled_logging), logging_period(logging_period), logging_file_path(logging_file_path)
{
	...
	void IO_Flow_Base::SATA_consume_io_request(Host_IO_Request* request)
	{
		...
		if (Simulator->Time() > next_logging_milestone) {
			log_file << Simulator->Time() / SIM_TIME_TO_MICROSECONDS_COEFF << "\t" << Get_device_response_time_short_term() << "\t" << Get_end_to_end_request_delay_short_term() << "\t" << STAT_serviced_request_count_short_term << std::endl;
			STAT_sum_device_response_time_short_term = 0;
	...
	void IO_Flow_Base::NVMe_consume_io_request(Completion_Queue_Entry* cqe)
	{
		...
		DEBUG7("Request with LBA =" << request->Start_LBA << " and Type =" << (request->Type == Host_IO_Request_Type::READ ? "READ" : "WRITE") << " finished at time " << Simulator->Time())
		sim_time_type device_response_time = Simulator->Time() - request->Enqueue_time;
		sim_time_type request_delay = Simulator->Time() - request->Arrival_time;
		STAT_serviced_request_count++;
		STAT_serviced_request_count_short_term++;
		...
		// existing_LBAs.erase(request->Start_LBA);
		...
		while(waiting_requests.size() > 0) {
			if (!NVME_SQ_FULL(nvme_queue_pair) && available_command_ids.size() > 0) {
				...
				// while (existing_LBAs.find(new_req->Start_LBA) != existing_LBAs.end()) {
				// 	conflict_requests.push_back(new_req);
				// 	if (waiting_requests.size() == 0) {
				// 		break;
				// 	}
				// 	new_req = waiting_requests.front();
				// 	waiting_requests.pop_front();
				// }
				// while (conflict_requests.size() > 0) {
				// 	waiting_requests.push_front(conflict_requests.back());
				// 	conflict_requests.pop_back();
				// }
				// if (existing_LBAs.find(new_req->Start_LBA) != existing_LBAs.end()) {
				// 	break;
				// }
				...
				DEBUG7("Host:: Request with LBA=" << new_req->Start_LBA  << " and LBA count = "<< request->LBA_count << " and Type=" << (new_req->Type == Host_IO_Request_Type::READ ? "READ" : "WRITE") << " ENQUEUED at time" << Simulator->Time())
				// existing_LBAs.insert(request->Start_LBA);
		...
		switch (SSD_device_type) {
			case HostInterface_Types::NVME:
				...
				if (NVME_SQ_FULL(nvme_queue_pair) || available_command_ids.size() == 0) {
					...
					// DEBUG11("Pending Requests!")
				// } else if (existing_LBAs.find(request->Start_LBA) != existing_LBAs.end()){
				// 	waiting_requests.push_back(request);
				} else {	
					if (nvme_software_request_queue[*available_command_ids.begin()] != NULL) {
					...
					// DEBUG11("Host:: Request with LBA=" << request->Start_LBA << " and LBA count = "<< request->LBA_count <<" and Type=" << (request->Type == Host_IO_Request_Type::READ ? "READ" : "WRITE") << " SUBMITTED at time" << Simulator->Time() << " Arrival Time" << request->Arrival_time)
					existing_LBAs.insert(request->Start_LBA);
	...
	float IO_Flow_Base::Get_device_response_time()
	{
		if (STAT_serviced_request_count == 0) {
			return 0;
		}

		return (float)((float)STAT_sum_device_response_time / (float)STAT_serviced_request_count /(float) SIM_TIME_TO_MICROSECONDS_COEFF);
	}

	uint32_t IO_Flow_Base::Get_device_response_time_read()
	{
	...
	uint32_t IO_Flow_Base::Get_device_response_time_write()
	{
	...
	void IO_Flow_Base::Report_results_in_XML(std::string name_prefix, Utils::XmlWriter& xmlwriter)
	{
		...
		val = std::to_string(STAT_serviced_request_count);
		xmlwriter.Write_attribute_string(attr, val);
		...
		attr = "IOPS";
		val = std::to_string((double)STAT_generated_request_count / (((double)Simulator->Time()) / ((double)SIM_TIME_TO_SECONDS_COEFF)));
		xmlwriter.Write_attribute_string(attr, val);

		attr = "IOPS_Read";
		val = std::to_string((double)STAT_generated_read_request_count / (((double)Simulator->Time()) / ((double)SIM_TIME_TO_SECONDS_COEFF)));
		xmlwriter.Write_attribute_string(attr, val);

		attr = "IOPS_Write";
		val = std::to_string((double)STAT_generated_write_request_count / (((double)Simulator->Time()) / ((double)SIM_TIME_TO_SECONDS_COEFF)));
		xmlwriter.Write_attribute_string(attr, val);
		...
		attr = "Bandwidth";
		// std::cout << "outputing bandwidth:" << STAT_transferred_bytes_total << " " << Simulator->Time() << " " << SIM_TIME_TO_SECONDS_COEFF;
		val = std::to_string((double)STAT_transferred_bytes_total / (((double)Simulator->Time()) / ((double)SIM_TIME_TO_SECONDS_COEFF)));
		xmlwriter.Write_attribute_string(attr, val);

		attr = "Bandwidth_Read";
		val = std::to_string((double)STAT_transferred_bytes_read / (((double)Simulator->Time()) / ((double)SIM_TIME_TO_SECONDS_COEFF)));
		xmlwriter.Write_attribute_string(attr, val);

		attr = "Bandwidth_Write";
		val = std::to_string((double)STAT_transferred_bytes_write / (((double)Simulator->Time()) / ((double)SIM_TIME_TO_SECONDS_COEFF)));
		xmlwriter.Write_attribute_string(attr, val);
		...
		attr = "Device_Response_Time_Read";
		val = std::to_string(Get_device_response_time_read());
		xmlwriter.Write_attribute_string(attr, val);


		attr = "Device_Response_Time_Write";
		val = std::to_string(Get_device_response_time_write());
		xmlwriter.Write_attribute_string(attr, val);
