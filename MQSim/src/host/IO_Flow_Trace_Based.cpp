...
IO_Flow_Trace_Based::IO_Flow_Trace_Based(const sim_object_id_type &name, uint16_t flow_id, LHA_type start_lsa_on_device, LHA_type end_lsa_on_device, uint16_t io_queue_id,
										 ...
										 bool enabled_logging, sim_time_type logging_period, std::string logging_file_path, double device_bandwidth) : IO_Flow_Base(name, flow_id, start_lsa_on_device, end_lsa_on_device, io_queue_id, nvme_submission_queue_size, nvme_completion_queue_size, priority_class, 0, initial_occupancy_ratio, 0, SSD_device_type, pcie_root_complex, sata_hba, enabled_logging, logging_period, logging_file_path),
																															  ...
																															  total_requests_in_file(0), time_offset(0), fileread_finished(false), estimated_workload_bandwidth(0), device_bandwidth(device_bandwidth)
{
...
void IO_Flow_Trace_Based::Fetch_new_timewindow() {
	DEBUG11("Fetch Time Window!");
	if (fileread_finished) {
		trace_lines_in_window.clear();
		return;
	}
	// fetch the first time window
	...
	if(current_trace_line.size() > 0) {
		// DEBUG11("Use existing Line:" << current_trace_line[0])
		new_line = current_trace_line;
	} else {
		std::getline(trace_file, trace_line);
		// DEBUG11("Get line:" << trace_line)
		Utils::Helper_Functions::Remove_cr(trace_line);
		Utils::Helper_Functions::Tokenize(trace_line, ASCIILineDelimiter, new_line);
	}
	trace_lines_in_window.push_back(new_line);
	// generate the time window metadata
	time_start = time_offset + std::strtoll(new_line[ASCIITraceTimeColumn].c_str(), &pEnd, 10);
	uint64_t time_start_fixed = time_start;
	uint64_t time_end = time_offset + time_start + PREFETCH_TIME_WINDOW;
	...
	while (time_start < time_end) {
		...
		if(std::getline(trace_file, trace_line)) {
			...
			trace_lines_in_window.push_back(new_line);
			time_start = time_offset + std::strtoll(new_line[ASCIITraceTimeColumn].c_str(), &pEnd, 10);
			bytes_transferred += std::strtoll(new_line[ASCIITraceSizeColumn].c_str(), &pEnd, 10) * SECTOR_SIZE_IN_BYTE;
		} else {
			break;
		}
	}
	if (time_end - time_start_fixed > 0) {
		estimated_workload_bandwidth = (double) bytes_transferred / ((double) (time_end - time_start_fixed));
	}
	DEBUG11("Bytes Transferred: " << bytes_transferred << " Time Interval: " << (double) (time_end - time_start_fixed) << "Estimated Workload Bandwidth = " << estimated_workload_bandwidth << " Real Device Bandwidth = " << device_bandwidth)
	current_trace_line.clear();
	if (time_start < time_end) {
		fileread_finished = true;
		trace_file.close();
	} else {
		current_trace_line = trace_lines_in_window.back();
		trace_lines_in_window.pop_back();
	}
	// DEBUG11("Time Window Fetched " << trace_lines_in_window.size());
	// calculate bandwidth
	// TODO what if the bandwidth is too low? 
}

Host_IO_Request *IO_Flow_Trace_Based::Generate_next_request()
{
	// DEBUG11("Generate Next Req!");
	if ( trace_lines_in_window.size() == 0  ) {
		// DEBUG11("NULL 1: " << trace_lines_in_window.size() << " " <<  fileread_finished);
		return NULL;
	}
	if (cur_traceline == trace_lines_in_window.size()) {
		if (fileread_finished) {
			return NULL;
		}
		Fetch_new_timewindow();
		return Generate_next_request();	
	}
	// DEBUG11("Generate!");
	vector<string> current_line = trace_lines_in_window[cur_traceline++];
	
	Host_IO_Request *request = new Host_IO_Request;
	if (current_line[ASCIITraceTypeColumn].compare(ASCIITraceWriteCode) == 0) {
	...
	char *pEnd;
	request->LBA_count = std::strtoul(current_line[ASCIITraceSizeColumn].c_str(), &pEnd, 0);

	request->Start_LBA = std::strtoull(current_line[ASCIITraceAddressColumn].c_str(), &pEnd, 0);
...
{
	// DEBUG11("Get Req Respond!");
	double trans_bytes = SECTOR_SIZE_IN_BYTE * (double)(nvme_software_request_queue[io_request->Command_Identifier]->LBA_count);
	sim_time_type device_response_time = Simulator->Time() - nvme_software_request_queue[io_request->Command_Identifier]->Enqueue_time;
	IO_Flow_Base::NVMe_consume_io_request(io_request);
	IO_Flow_Base::NVMe_update_and_submit_completion_queue_tail();
	// Fire a new reqeust, within computation delay.
	if (STAT_generated_request_count < total_requests_to_be_generated)
	{
		// calculate the corresponding computation delay
		double computation_delay_d =  (double) (trans_bytes) / ((double)estimated_workload_bandwidth) - (double) (trans_bytes) / (device_bandwidth);
		uint64_t computation_delay = 0;
		if (computation_delay_d > 0) {
			computation_delay = (uint64_t) ceil(computation_delay_d);
		}
		// DEBUG11("Fire another Req, Device respond time:" << device_response_time << ", computation delay:" << computation_delay );
		char *pEnd;
		Simulator->Register_sim_event(Simulator->Time() + computation_delay, this);
	}
...
{
	double trans_bytes = SECTOR_SIZE_IN_BYTE * (double)(io_request->LBA_count);
...
void IO_Flow_Trace_Based::Start_simulation()
{
	...
	// DEBUG11("Firing Initial Requests!");
	// assume that the paralellism can be fully controlled by queue depth 
	while (!NVME_SQ_FULL(nvme_queue_pair) && cur_traceline < trace_lines_in_window.size()) {
		// DEBUG11("Fire!");
										 ...
										 page_status_type (*Find_NVM_subunit_access_bitmap)(LHA_type lha))
{
	...
	while (std::getline(trace_file_temp, trace_line))
	{
		...
		if (stats.Total_generated_requests % 10000 == 0) {
			std::cout << "\rCurrent Preconditioning Request Number: " << stats.Total_generated_requests << std::flush;
		}
	}
	std::cout << std::endl;
