		...
		switch ((HBA_Sim_Events)event->Type)
		{
			case HBA_Sim_Events::CONSUME_IO_REQUEST:
			{
				...
				while (waiting_requests_for_submission.size() > 0) {
					if (!SATA_SQ_FULL(sata_ncq) && available_command_ids.size() > 0)
					{
						...
						DEBUG6("Request with LBA=" << new_req->Start_LBA  << " and LBA count = "<< request->LBA_count <<" and Type=" << (new_req->Type == Host_IO_Request_Type::READ ? "READ" : "WRITE") << " ENQUEUED at time" << Simulator->Time())
			...
			case HBA_Sim_Events::SUBMIT_IO_REQUEST:
			{
				...
				if (SATA_SQ_FULL(sata_ncq) || available_command_ids.size() == 0) {
					...
					DEBUG6("Request with LBA=" << request->Start_LBA << " and LBA count = "<< request->LBA_count <<" and Type=" << (request->Type == Host_IO_Request_Type::READ ? "READ" : "WRITE") << " ENQUEUED at time" << Simulator->Time())
