...
#define NVME_UPDATE_SQ_TAIL(Q)  Q.Submission_queue_tail++;\
	...
	public:
		...
		float Get_device_response_time();//in microseconds
		uint32_t Get_device_response_time_read();//in microseconds
		uint32_t Get_device_response_time_write();//in microseconds
	...
	protected:
		...
		std::set<LHA_type> existing_LBAs;
		std::list<Host_IO_Request*> conflict_requests;
