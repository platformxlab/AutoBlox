	...
	void PCIe_Link::Deliver(PCIe_Message* message)
	{
		switch (message->Destination) {
			case PCIe_Destination_Type::HOST://Message from SSD device to the host
				...
				DEBUG6("Estimate transfer time for message to HOST:size=" << message->Payload_size<< "; address=" << message->Address << " Type=" << ((message->Type == PCIe_Message_Type::READ_REQ) ? "READ" : ((message->Type == PCIe_Message_Type::WRITE_REQ) ? "WRITE" : "READ_COMP"))  << " Destination="  << "is from "<< Simulator->Time() << " to "<<  Simulator->Time() + estimate_transfer_time(message))
				break;
			case PCIe_Destination_Type::DEVICE://Message from Host to the SSD device
				...
				DEBUG6("Estimate transfer time for message to DEVICE: size=" << message->Payload_size<< "; address=" << message->Address << " Type=" << ((message->Type == PCIe_Message_Type::READ_REQ) ? "READ" : ((message->Type == PCIe_Message_Type::WRITE_REQ) ? "WRITE" : "READ_COMP")) << " Destination=" << "is from "<< Simulator->Time() << " to "<<  Simulator->Time() + estimate_transfer_time(message))
				break;
			default:
				break;
		}
		...
		switch (destination) {
			case PCIe_Destination_Type::HOST:
				...
				if (Message_buffer_toward_root_complex.size() > 0) {//There are active transfers
					...
					DEBUG6("Estimate transfer time for message to HOST:size=" << message->Payload_size<< "; address=" << message->Address << " Type=" << ((message->Type == PCIe_Message_Type::READ_REQ) ? "READ" : ((message->Type == PCIe_Message_Type::WRITE_REQ) ? "WRITE" : "READ_COMP"))  << " Destination="  << "is from "<< Simulator->Time() << " to "<<  Simulator->Time() + estimate_transfer_time(Message_buffer_toward_root_complex.front()))
				}
				break;
			case PCIe_Destination_Type::DEVICE:
				...
				if (Message_buffer_toward_ssd_device.size() > 0) {
					...
					DEBUG6("Estimate transfer time for message to DEVICE:size=" << message->Payload_size<< "; address=" << message->Address << " Type=" << ((message->Type == PCIe_Message_Type::READ_REQ) ? "READ" : ((message->Type == PCIe_Message_Type::WRITE_REQ) ? "WRITE" : "READ_COMP"))  << " Destination="  << "is from "<< Simulator->Time() << " to "<<  Simulator->Time() + estimate_transfer_time(Message_buffer_toward_ssd_device.front()))
				}
				break;
		}
	}
